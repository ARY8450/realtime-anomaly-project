...
def start_dashboard_subprocess(port=None):
    # Launch Streamlit in a subprocess so the script can keep running
    cmd = ["streamlit", "run", "realtime_anomaly_project/app/dashboard.py"]
    if port:
    ...
    try:
        ...
        print("Error: 'streamlit' CLI not found on PATH. Activate venv or install streamlit.")
        return None
